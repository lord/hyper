...
fn map_response_to_message(mut res: Response) -> Message<ResponseHead, Receiver<http::Chunk, ::Error>> {
    use futures::stream::channel;
    if let Some(body) = res.body {
        //Message::WithBody(res, res.body.take().unwrap().to_vec())
        //let (tx, rx) = channel();
        //tx.send(Ok(res.body.take().unwrap().into())).poll();
        Message::WithBody(res.head, body)
    } else {
        Message::WithoutBody(res.head)
    }
}
    ...
    where T: Service<Request=Request, Response=Response, Error=::Error>,
{
    ...
    type Response = Message<ResponseHead, Receiver<http::Chunk, ::Error>>;
    type Error = ::Error;
    type Future = Map<T::Future, fn(Response) -> Message<ResponseHead, Receiver<http::Chunk, ::Error>>>;

    fn call(&self, message: Self::Request) -> Self::Future {
        let req = match message {
            Message::WithoutBody(head) => Request::new(head),
            Message::WithBody(head, body) => unimplemented!("Message::WithBody")
        };
